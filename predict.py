...
def get_response(intents_list, intents_json,message=""):
    ...
    for i in list_of_intents:
        if i['topics'] == tag:
            print(tag)
            if tag=='shoes' or tag=='jeans' or tag=='shirts':
                
                url_list=getFlickrResult(tag)
                result=random.choice(url_list)
            elif tag=='clothes':
                
                url_list=getFlickrResult('clothes')
                result=random.choice(url_list)
            elif tag=="who what" or tag=="when" or tag=="why" or tag=="how":
                
                res=getWikiResult(message)
                if len(res)>1:
                    result=res[0]
